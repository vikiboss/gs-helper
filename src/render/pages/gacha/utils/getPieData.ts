...
const getPieData = (type: 'star' | 'item' | 'type', list: GachaData['list']) => {
  switch (type) {
    ...
    case 'type': {
      ...
      for (const { uigf_gacha_type: type } of list) {
        type === GachaTypeMap.activity && t1.value++;
        type === GachaTypeMap.weapon && t2.value++;
        type === GachaTypeMap.normal && t3.value++;
        type === GachaTypeMap.newer && t4.value++;
      }
      return [t1, t2, t3, t4];
    }
  }
