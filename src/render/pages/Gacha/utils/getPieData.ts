...
const getPieData = (type: "star" | "item" | "type", list: GachaData["list"]) => {
  switch (type) {
    ...
    case "type": {
      [t1, t2, t3, t4].forEach((e) => (e.value = 0));
      for (const item of list) {
        const acts = GachaTypeMap["activity"];
        for (const e of acts) if (item.gacha_type === e) t1.value++;
        if (item.gacha_type === GachaTypeMap["weapon"]) t2.value++;
        if (item.gacha_type === GachaTypeMap["normal"]) t3.value++;
        if (item.gacha_type === GachaTypeMap["newer"]) t4.value++;
      }
      return [t1, t2, t3, t4];
    }
  }
