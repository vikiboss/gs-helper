...
const Overview: React.FC<PageProp> = ({ gacha }) => {
  const MostInfo = useMemo(() => getMostInfo(gacha), gacha.list);
  const statictics = useMemo(() => getGachaStatictics(gacha), gacha.list);
  const luckInfo = useMemo(() => getLuckInfo(gacha), gacha.list);
  const limitRoleTimes = useMemo(() => getAverageTimes(gacha, "role"), gacha.list);
  const limitWeaponTimes = useMemo(() => getAverageTimes(gacha, "weapon"), gacha.list);
