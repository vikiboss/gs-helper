...
const UserCard: React.FC<UserCardProp> = (props) => {
  ...
  const infos = [
    ...
    },
    {
      key: 'region',
      name: '区服',
      content: `${user.region_name} （${
        user.is_official ? '官服' : '渠道服'
      }）`,
    },
    {
      key: 'uid',
  ...
  const isResinToday =
    new Date(Date.now() + resinTime * 1000).getDay() === new Date().getDay();
  const resinTimeText = `将于${isResinToday ? '今日' : '明日'} ${targetTime} `;
  const resinText = `下次恢复还剩${nextTime}，${resinTimeText}回满（${formatTime(
    resinTime
  )}）`;
  ...
  // 处理周本数据
  const { remain_resin_discount_num: remain, resin_discount_num_limit: limit } =
    note;
  ...
  const dispatchReadyTime = D(Date.now() + lastDispatchTime * 1000).format(
    'HH:mm:ss'
  );
  const dispatchTimeText =
    (isDispatchToday ? '今日 ' : '明日 ') + dispatchReadyTime;
  const dispatcTitle =
    dispatchs.length > 0
      ? isDispatchAllReady
        ? '已全部完成，等待领取'
    ...
    <>
      ...
      <div className={styles.noteCard}>
        {notes.length &&
          notes.map((e) => 
            ...
            >
              <img
                src={e.icon}
                className={cn(styles.noteIcon, styles[e.name])}
              />
        ...
        <div
          ...
          title={dispatcTitle}
          onClick={() =>
            notice[isDispatchAllReady ? 'success' : 'info']({
              message: dispatcTitle,
            })
          }
        >
