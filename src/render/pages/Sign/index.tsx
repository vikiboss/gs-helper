...
const Sign: React.FC = () => {
  ...
  const handleSign = async () => {
    if (signInfo.is_sign) {
      notice.faild({ message: "今天已经签过到啦~ 不要重复签到哦" });
    } else {
      ...
      if (isSignDone) {
        ...
        notice.success({ message: `签到成功！获得 ${todayAward}，已签到 ${total + 1} 天` });
        await updateInfo();
      } else {
  ...
  return (
    <>
      <div className={styles.desc}>
        ...
        >{`米游社 ${signData.month} 月签到日历，共签到 ${signInfo.total_sign_day} 天，漏签 ${signInfo.sign_cnt_missed} 天`}</div>
